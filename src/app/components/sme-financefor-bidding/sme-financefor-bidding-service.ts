...
export class SmeFinancierForBiddingServices {
  ...
  getFinanceForBiddingLists(){
    return this.apiService.tempGet('http://localhost:8080/invoice-request/allInvoicesBySmeId/'+localStorage.getItem("userId"));
  }
  getInvoiceRequestLists(id){
    // let stringifyObj = JSON.stringify( { invoiceDetails : { id : 1} })
    // debugger;
    return this.apiService.tempGet('http://localhost:8080/invoice-request/getInvoiceData/'+id); 
  }
  ...
  getFinanceBiddingLists(id){
    ...
    return this.apiService.tempGet('http://localhost:8081/api/v1/bidding-details/getBiddingDetails/'+id);
  }
